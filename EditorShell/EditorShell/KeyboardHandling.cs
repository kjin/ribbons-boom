...
namespace EditorUILibrary
{
    class KeyboardHandling
    {
        ...
        public static bool TryConvertKeyboardInput(KeyboardState keyboard, KeyboardState oldKeyboard, out string key)
        {
            ...
            {
                if (!oldKeyboard.IsKeyDown(keys[i]) && !alt && !control)
                {
                    switch (keys[i])
                    {
                        //Alphabet keys
                        ...
                        case Keys.OemQuotes: if (shift) { key = '"'.ToString(); } else { key = "\'"; } return true;
